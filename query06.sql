...
wheelchair_stops_per_neighborhood as (
	select n."NAME", COUNT(n.stop_id) as inaccessible_count
	from bus_stop_neighborhoods as n
	where n.wheelchair_boarding = 1
	group by n."NAME"
),
base_metrics as (
	select n."NAME", total.total_count as num_bus_stops_accessible, wheelchair.inaccessible_count as num_bus_stops_inaccessible, (wheelchair.inaccessible_count/(total.total_count*1.0)) as percent_accessible, (total.total_count/n."Shape_Area")*100000 as stop_density
	from neighborhoods_philadelphia as n
